            ...
            return <TabPane tab={<img src={heThongRap.logo} className="rounded-full" width={40} alt="img" />} key={index}>
                ...
                <Tabs className="tabDiaDiem overflow-y-scroll" style={{ height: '450px', width: '100%' }} tabPosition={tabPosition} >
                    {heThongRap.lstCumRap?.map((cumRap, index) => {
                        return <TabPane className="overflow-y-scroll" style={{ height: '450px' }} tab={
                            <div className="flex justify-start" style={{ width: '300px' }}>
                                ...
                                <div className="text-left">
                                    ...
                                    <h1 className="mb-0" style={{color:'#8bc541'}}>{cumRap.tenCumRap}</h1>
                                    <p className="mb-0" style={{fontSize:'12px'}} >{cumRap.diaChi}</p>
                                </div>
                            </div>
                        } key={index}>
                            {/* {noi dung tab } */}
                                    ...
                                    <div  key={index}>
                                        <div className="flex">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <p className="text-xs text-gray-400">{cumRap.diaChi}</p>
                                            </div>
                                        </div>

                                        <div className="grid grid-cols-5 gap-6 my-5 pr-10">
                                            {phim.lstLichChieuTheoPhim?.slice(0, 12).map((lichChieu, index) => {
                                                return <NavLink to={`/checkout/${lichChieu.maLichChieu}`} className="button-30 font-bold hover:text-green-500">
